...
(`PresentationBuilder`) и отправляет файл в чат. Теги — простые слова через запятую/пробел (без
категорий), с резервом на синонимы (`TagTaxonomy`, `config/tags.php`) — свободный текст всей фразы
целиком (`QueryParser`) ещё не реализован:
```
кейсы ритейл
