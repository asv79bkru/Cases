...
`php bin/index.php` обходит `presentations/`, извлекает текст слайдов (`python/slide_text_extractor.py`),
предлагает теги по `config/tags.php` и спрашивает подтверждение/правку по каждому слайду за один проход
(Enter — принять предложенное, `skip` — пропустить слайд, свои теги — `категория:тег[,категория:тег...]`),
плюс необязательную ссылку на сайт (`site_url`). Повторный запуск обновляет те же записи, не создавая дублей.
